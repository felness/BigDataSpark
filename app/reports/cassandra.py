...
def reports_cassandra(spark, config):
    """
    ...
    """
    # Настройка Cassandra
    spark.conf.set("spark.cassandra.connection.host", config["datasource"]["cassandra"]["contact-points"])
    spark.conf.set("spark.cassandra.connection.port", config["datasource"]["cassandra"]["port"])
    spark.conf.set("spark.cassandra.auth.username", config["datasource"]["cassandra"]["username"])
    spark.conf.set("spark.cassandra.auth.password", config["datasource"]["cassandra"]["password"])
    ...
    logger.info("Создание витрины продаж по продуктам...")
    ...
    write_cassandra(sales_by_product, config, "sales_by_product")

    # Витрина продаж по клиентам
    logger.info("Создание витрины продаж по клиентам...")
    ...
    write_cassandra(sales_by_customer, config, "sales_by_customer")

    # Витрина продаж по времени
    logger.info("Создание витрины продаж по времени...")
    sales_by_time_query = """
    ...
    write_cassandra(sales_by_time, config, "sales_by_time")

    # Витрина продаж по магазинам
    logger.info("Создание витрины продаж по магазинам...")
    ...
    write_cassandra(sales_by_store, config, "sales_by_store")

    # Витрина продаж по поставщикам
    logger.info("Создание витрины продаж по поставщикам...")
    ...
    write_cassandra(sales_by_supplier, config, "sales_by_supplier")

    # Витрина качества продукции
    logger.info("Создание витрины качества продукции...")
    ...
    write_cassandra(product_quality, config, "product_quality")

    logger.info("Все витрины успешно созданы в Cassandra")
...
def write_cassandra(df, config, table):
    """
    ...
    df.write \
        ...
        .option("keyspace", config["datasource"]["cassandra"]["keyspace"]) \
        .option("table", table) \
        .mode("append") \
        .save()